...
The replication package (\url{https://github.com/jbarseneau99/papers/tree/main/an-autogenic-agent}) releases the ontology-catalog snapshot, the enum-locking schema migrations, the \texttt{M33-VEGA-INST-001} chain, the coherence-check design specification, and the paper source; Vega and Boole themselves remain proprietary.
...
\textbf{Fully autonomous closes were zero.} The 159 shipped break down by
\texttt{proposed\_by} (Table \ref{tab:findings-by-path}, Fig. \ref{fig:throughput}): 144 proposed by the
agent (91\%), 15 by a user (9\%), 0 by team-decomposition.

Attribution is by \texttt{proposed\_by}: 144 agent-proposed, 15 user-proposed.
All 159 crossed the same principal-review gate
(§\ref{sec:ontic-gate}); a fix-proposal join on \texttt{finding\_id}
covers 15 shipped rows. The registry's per-Finding fields
(\texttt{proposed\_at}, \texttt{state}, \texttt{proposed\_by}, \texttt{fix\_proposals.finding\_id})
