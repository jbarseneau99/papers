...
% Tighten float spacing to recover vertical space around figures — IEEEtran
% defaults are generous (20pt \textfloatsep, 12pt \floatsep / \intextsep,
% 10pt \abovecaptionskip). At column width three figures each cost several
% lines to those gaps; tightening pulls ~1 body line per figure back.
...
% Force Fig 2 (mechanism) onto the page after Fig 1 lands: wrap it in
% \afterpage so LaTeX defers placement to the following page.
...
structural access (§\ref{sec:related}). The agent must rediscover them each session, by search. Two
...
neuroscientific equivalence. The architecture is \emph{autogenic}: its
...
In Vega, the ontology is a \emph{catalog of classes}: one hundred and
fourteen declared across eight disciplines at the time of writing
(per-discipline counts in Fig.\textasciitilde{}\ref{fig:ontology}), one
hundred and twelve with a machine schema, each defined in
\texttt{lib/proto/*.js} and registered in a single manifest. The formal catalog was created on 6 July 2026.
...
express: a new \texttt{Message} on an existing \texttt{Channel}, a new \texttt{Finding}
reaching \texttt{shipped} through the enumerated transitions, a new
\texttt{TelemetryEvent} row emitted by an existing \texttt{Sensor}. Each is a new
...
consistent with the existing requirement and telemetry. Instances are the ordinary growth. Six roles separate for a shipped Instance: origination, proposal, implementation (agent); construction-time validation (kernel, ADR 0012); approval and deployment supervision (§\ref{sec:ontic-gate}); closure. Only the kernel is automated.

Table \ref{tab:findings-by-path} classifies a shipped Finding as \textbf{Instance} if its closure diff added no migration, touched no locked file, and never transitioned through \texttt{needs\_external\_dev}; \textbf{Ontic-break} if any of those three occur; \textbf{Unknown} if no affected-file record exists.
...
scope. At the workflow layer, the Feature-Driven Development (FDD) lifecycle \cite{palmer2002} classifies affected
...
The coherence-triad check (§\ref{sec:triad}) is a separate
designed instrument: a boolean predicate the schema carries as a
\texttt{CHECK} or foreign-key constraint (or an ontology-table query),
returning \emph{holds} or \emph{fails} against the failing instance.
...
(§\ref{sec:res-attribution}). A prospective matched evaluation (baseline / code-graph / ontology, held constant on model, tasks, tools, budget, and policy) is proposed future work.
